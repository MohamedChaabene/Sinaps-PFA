...
async function seed() {
  try {
    ...
    if (error.name === 'MongooseServerSelectionError' || (error.message && error.message.includes('ECONNREFUSED'))) {
      console.error(`\n❌ Impossible de se connecter à MongoDB sur ${MONGO_URI} (serveur local non démarré).`);
      console.log('\n💡 NOTE : Vous n\'avez pas besoin d\'installer MongoDB !');
      console.log('   Lancez simplement : npm run dev');
      console.log('   Le serveur démarre automatiquement une base de données en mémoire avec toutes les données de démo pré-chargées.\n');
    } else {
...
module.exports = { seed, populateDemoData };
