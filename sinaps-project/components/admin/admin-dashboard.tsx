...
function AdminSidebar({ active, onClose }: { active: string; onClose?: () => void }) {
  ...
  return <aside className="flex h-full w-72 shrink-0 flex-col border-r border-sidebar-border bg-sidebar px-5 py-6"><div className="flex items-center justify-between"><Link href="/" className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-2xl bg-primary font-heading text-lg font-extrabold text-primary-foreground">S</div><span className="font-heading text-xl font-bold">Supportly</span></Link>{onClose && <Button variant="ghost" size="icon-sm" onClick={onClose} aria-label="Fermer"><X /></Button>}</div><Separator className="my-7" /><nav className="flex flex-col gap-2" aria-label="Navigation administration">{items.map(([id, label, Icon]) => <a key={id} href={`#${id}`} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${active === id ? "bg-secondary text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}><Icon className="size-4" />{label}</a>)}</nav><div className="mt-auto rounded-2xl bg-secondary/60 p-4"><ShieldCheck className="mb-3 size-5 text-primary" /><p className="text-sm font-semibold">Espace sécurisé</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Gérez les accès de votre équipe support.</p><Button variant="outline" size="sm" onClick={() => logout(router)} className="mt-3 w-full justify-start gap-2"><LogOut className="size-4" />Déconnexion</Button></div></aside>
}
