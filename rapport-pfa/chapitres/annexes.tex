\chapter{Annexes}

\section{Glossaire des statuts de conversation}

\begin{table}[H]
\centering
\caption{Statuts et modes de prise en charge}
\label{tab:annexe-statuts}
\begin{tabularx}{\textwidth}{@{}llX@{}}
\toprule
Champ & Valeur & Signification opérationnelle \\
\midrule
status & \texttt{en\_cours} & Conversation active (IA ou humain déjà en charge) \\
status & \texttt{en\_attente} & Escaladée, en file côté agents \\
status & \texttt{resolu} & Clôturée \\
handledBy & \texttt{ia} & Prochaine réponse automatique possible \\
handledBy & \texttt{humain} & Réponses attendues d'un opérateur \\
\bottomrule
\end{tabularx}
\end{table}

\section{Exemple d'entrée de la base de connaissances}

Les extraits suivants sont des \textbf{exemples} utilisés par le retriever, et non une documentation officielle de produits SINAPS.

\begin{lstlisting}[caption={Extrait de \texttt{knowledgeBase.js}}]
{
  question: "Comment suivre ma commande ?",
  answer: "Vous pouvez suivre votre commande depuis votre espace
           client, section 'Mes commandes'. Un numéro de suivi
           est envoyé par email dès l'expédition."
}
\end{lstlisting}

\section{Comptes du script de seed (environnement de démo)}

Ces identifiants figurent dans le README du projet à des fins de démonstration locale. Ils ne doivent pas être réutilisés en production.

\begin{table}[H]
\centering
\caption{Comptes créés par \texttt{npm run seed}}
\label{tab:seed}
\begin{tabularx}{\textwidth}{@{}lllX@{}}
\toprule
Rôle & E-mail & Mot de passe (seed) & Accès \\
\midrule
Admin & admin@sinaps.com & password123 & \texttt{/admin} \\
Agent & sarah.benali@sinaps.com & password123 & \texttt{/agent} \\
Agent & karim.mansouri@sinaps.com & password123 & \texttt{/agent} \\
Pending & youssef.mehdi@sinaps.com & password123 & login refusé tant que non validé \\
\bottomrule
\end{tabularx}
\end{table}

\section{Checklist de captures d'écran}

À cocher après insertion aux emplacements indiqués au chapitre~4 :

\begin{itemize}[leftmargin=1.4em]
  \item[$\square$] Important : chat IA et actions rapides (\texttt{fig:client-chat-ai})
  \item[$\square$] Important : escalade, file et message humain (\texttt{fig:human-escalation})
  \item[$\square$] Important : dashboard admin (\texttt{fig:admin-dashboard})
  \item[$\square$] Important : dialogue de satisfaction (\texttt{fig:satisfaction-dialog})
  \item[$\square$] Optionnel : identification client (\texttt{fig:client-auth})
  \item[$\square$] Optionnel : pièce jointe et avatars (\texttt{fig:message-attachment-avatar})
  \item[$\square$] Optionnel : inscription et validation agent (\texttt{fig:agent-approval})
\end{itemize}

\section{Compilation de ce rapport}

\begin{lstlisting}[caption={Compilation LaTeX}]
cd rapport-pfa
pdflatex main.tex
pdflatex main.tex
\end{lstlisting}

Placer éventuellement un logo ISIMS officiel sous \texttt{images/logo-isims.jpg}. Le fichier \texttt{images/logo-organisme.jpg} correspond au visuel fourni pour l'organisme / l'identité du projet.
