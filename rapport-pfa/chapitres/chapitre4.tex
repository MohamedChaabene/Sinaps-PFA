...
Mobile & Expo 57, React Native 0.86, React 19, TypeScript, Socket.io-client, AsyncStorage, Expo Auth Session, Image Picker et Document Picker \\
...
\textbf{Justification succincte.} Next.js fournit le routage et un frontend React moderne, conformément à l'orientation \og React \fg{} du sujet, tout en restant une application web. Expo / React Native fournit un client mobile distinct réutilisant le backend commun. Express est l'API demandée côté Node. MongoDB Atlas évite d'administrer un serveur local de production. Gemini a été retenu (et utilisé effectivement) plutôt qu'OpenAI. Socket.io couvre le besoin de réactivité du chat et de la file agent. JWT sépare clairement sessions client et sessions agent.

Les alternatives du sujet (Vue.js, Flutter, Firebase, Auth0, Kafka, Docker, Kubernetes, LangChain) \textbf{n'ont pas été mises en œuvre}. React Native est utilisé uniquement dans le client mobile Expo présent dans le dépôt.

\section{Organisation du code}

Le dépôt est organisé autour de trois applications ou sous-projets principaux : le frontend web, le backend et le client mobile.
...
\texttt{sinaps-mobile/src/context/} & Contextes d'authentification, de configuration, de thème et de conversation \\
\bottomrule
\end{tabularx}
\end{table}
...
\texttt{getStats} calcule : nombre total de conversations, résolues par IA, résolues par un humain, moyenne des notes renseignées, et un \textbf{temps de réponse moyen} (écart entre le premier message client et la première réponse IA ou humaine), en une passe sur les messages plutôt qu'en boucle na\"ive requête par conversation. Ces valeurs sont des indicateurs opérationnels orientés SLA, et non la preuve du respect d'un contrat SLA formel.
...
Le fichier \texttt{geminiService.js} enchaîne : retrieval $\rightarrow$ construction du prompt $\rightarrow$ essai de modèles Gemini $\rightarrow$ repli RAG textuel. La FAQ d'exemple couvre notamment : suivi de commande, retard, remboursement, mot de passe, code promo. Pour une question de facturation, seuls les documents de catégorie \texttt{billing} sont conservés ; en l'absence de document pertinent, le service propose une mise en relation humaine plutôt que d'inventer une réponse. Elle n'est pas un corpus métier confidentiel de SINAPS TEC.
...
Le dossier \texttt{sinaps-mobile/} contient un client Expo / React Native connecté aux mêmes routes HTTP et au même serveur Socket.io que le client web. Il propose une authentification client par nom / e-mail ou Google, une authentification agent / administrateur par e-mail et mot de passe, la persistance de session, le chat, les pièces jointes, les indicateurs de frappe, l'escalade, le retour vers l'IA, la clôture avec satisfaction et le mode sombre. Des écrans distincts permettent également à un agent de consulter la file, de prendre en charge une conversation, de répondre et de la résoudre, et à un administrateur de consulter les statistiques, les agents et l'historique.

Le client mobile a été vérifié par \texttt{npx tsc --noEmit}, mais aucune suite de tests automatisés ni recette formelle mobile n'est présente dans le dépôt. Les fonctionnalités décrites ci-dessus sont donc distinguées de la validation backend documentée au chapitre~5.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Important --- Conversation client et actions de statut}. La capture fournie montre les commandes de retour à l'IA et de clôture ; elle ne suffit pas à prouver visuellement une escalade ou une réponse humaine. Placement : après le paragraphe sur l'espace agent. Caption : \textit{Écran de conversation client avec commandes de retour à l'IA et de clôture}. Label : \texttt{fig:human-escalation}. Une nouvelle capture est recommandée pour documenter la file agent et un message humain.
  \item \textbf{Important --- Tableau de bord administrateur}. Capture de \texttt{/admin} montrant les KPI, la validation des agents et l'historique filtrable. Placement : après le paragraphe sur l'espace administrateur. Caption : \textit{Tableau de bord administrateur de SINAPS TEC}. Label : \texttt{fig:admin-dashboard}.
